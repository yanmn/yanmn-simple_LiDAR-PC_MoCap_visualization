...
gt_smpl_color = plt.get_cmap("tab20")(1)[:3]
smpl_color = plt.get_cmap("tab20")(2)[:3]
...
def load_npy_vis(file_path, start=0, end=-1, points='point_clouds', pose='pred_rotmats', remote=False):
    import pickle
...
if __name__ == '__main__':    
    ...
    parser.add_argument("--file_path", '-F', type=str,
                        default='D:/LiDARCap_extend/temp/MMSD2024/20230815Difeng_Badminton/CYM002/CYM002_label.h5py')

    args, opts = parser.parse_known_args()

    load_hdf5_vis(args.file_path, args.start, args.end)

    # if '/x' in args.file_path :
    #     converted_path  = args.file_path.replace('/x', '//202')
    # else:
    #     converted_path = args.file_path


    # if converted_path.endswith('.pkl'):
    #     load_pkl_vis(converted_path, args.start, args.end, remote=args.remote)
    # else:
    #     load_hdf5_vis(converted_path, args.start, args.end)
